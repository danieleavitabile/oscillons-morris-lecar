function branch = SecantContinuation(problem,u0,p0,stepperPars)
   ...
   if isempty(stepperPars.PlotBranchVariableId)
     iPlotBranchVariable = 3;
   else
     iPlotBranchVariable = 3 + stepperPars.PlotBranchVariableId;
   end
   ...
   function DisplayStep(step,u,p,status)
     ...
     if strcmp(status,'init') 
       ...
       outString = [outString sprintf('%9s %16s %14.6s %14.4s','STEP','PAR','2-NORM')];
     ...
     if numBranchVariables == 0
       outString = sprintf(formatString, step, p(iContPar), norm(u));
     else
       outString = sprintf(formatString, step, p(iContPar), norm(u), BranchVariables(step,u,p));
     end
     fprintf(outString);
   ...
   function SaveBranch(step,u,p,status)
     ...
     if strcmp(status,'init') 
       if numBranchVariables == 0
	 branch = [step p(iContPar) norm(u)];
       else
	 branch = [step p(iContPar) norm(u) BranchVariables(step,u,p)];
       end
     else
       if numBranchVariables == 0
	 branch = [branch; step p(iContPar) norm(u)];
       else
	 branch = [branch; step p(iContPar) norm(u) BranchVariables(step,u,p)];
       end
     end
   ...
   function plotHandle = PlotBranch(branch,idVar,parentHandle)
     ...
     figure(parentHandle)
     plot(branch(:,2),branch(:,idVar),'b.-',...
          branch(end,2),branch(end,idVar),'r.');
     drawnow;
