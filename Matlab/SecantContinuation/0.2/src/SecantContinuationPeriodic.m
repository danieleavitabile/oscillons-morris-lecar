function branch = SecantContinuationPeriodic(problem,u0,p0,stepperPars)
   ...
   fsolveOptionsConverge = fsolveOptions;
   fsolveOptionsConverge.display = 1;

   [u0,fval,exitflag,outputIterations] = NewtonSolver( @(u) problem(u,p0,uRef), u0, fsolveOptionsConverge );
   ...
   p1 = p0; p1(iContPar) = p0(iContPar) + s0/10;
   %[u1,fval,exitflag,output] = fsolve( @(u) problem(u,p1), u0, fsolveOptionsConverge );
   [u1,fval,exitflag,outputIterations] = NewtonSolver( @(u) problem(u,p1,uRef), u0, fsolveOptionsConverge );
   if exitflag <=0
   ...
   while step < maxSteps && ~continuationFailed && ~reachedBound
     ...
     [v,fval,exitflag,outputIterations]  = NewtonSolver( @(v) SecantCorrector(problem,v,uRef), v, fsolveOptions );

     % Successful step
     if exitflag > 0 

       % Step control
       xi = optNonlinIter/outputIterations; 
       if xi < 0.5
	 xi = 0.5;
       elseif xi > 2;
	 xi = 2;
   ...
   function [F,J] = SecantCorrector(problem,v,uRef)
     ...
     % Right-hand side
     if nargout > 1
       [F(iU),J(iU,iU)] = problem(v(iU),p,uRef);
     else
       F(iU) = problem(v(iU),p,uRef);
     end
     F(iP) = secant' * (v - v1) - s;
     ...
     if nargout > 1
       ...
       J(:,iP) = ( problem(v(iU),p,uRef) - F(iU) ) / epsi;
       J(iP,:) = secant';
       J = sparse(J);
     end
